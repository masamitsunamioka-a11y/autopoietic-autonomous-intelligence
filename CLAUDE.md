...
- **specification**: Minimal domain interfaces, Kandel-grounded — see [architecture.md](architecture.md);
  ...
  `working` (Memory, Episode, Knowledge)
...
- **runtime**: Core engines (Cortex, Thalamus, Plasticity, Drive) + Repository<I,E> SPI; CDI annotations are `provided`
...
filesystem/
  ...
  hippocampus/  — Persistent memory; {session}/episode.json (episodic) + knowledge.json (semantic, shared)
```
