...
- Generate new cognitive structures (Neurons, Modules) autonomously when capability gaps are detected
...
2. **Potentiate** — detect capability gaps; self-generate Neurons, Modules, and Effector skeletons
...
- **specification**: Minimal domain interfaces (Neuron, Module, Effector, Stimulus, Percept, Cortex, Thalamus, Nucleus,
  Plasticity, Drive, Memory)
...
filesystem/
  neural/
    ...
    modules/    — Module definitions (JSON, may reference effectors)
  signal/
...
- `anticorruption.modules.source` → `filesystem/neural/modules`
- `anticorruption.encodings.phase` → `filesystem/signal/phase`
- `anticorruption.encodings.shared` → `filesystem/signal/shared`
