...
public class TransmissionService implements Service<Impulse, Potential> {
    ...
    private static class Promptifier {
        ...
        public Promptifier(Serializer serializer,
            ...
            this.functionalExtern = new LocalFileSystem(Path.of(configuration.scope("synaptic").scope("function").get("source"), ""));
            this.sharedExtern = new LocalFileSystem(Path.of(configuration.scope("synaptic").scope("shared").get("source"), ""));
        }
        ...
        private List<Area> areaNonInnate() {
            return this.areaRepository.findAll().stream().filter(x -> !INNATE_AREAS.contains(x.id())).toList();
        }

        private List<Neuron> neuronSelf(String efferent) {
            return this.areaRepository.find(efferent).neurons().stream().map(this.neuronRepository::find).filter(Objects::nonNull).toList();
        }
        ...
        private List<Effector> effectorSelf(String efferent) {
            return this.areaRepository.find(efferent).effectors().stream().map(this.effectorRepository::find).filter(Objects::nonNull).toList();
        }
