...
<template>
  <div class="section">
    ...
    <div class="section-body">
      <div v-if="knowledge.length === 0" class="empty">
        No knowledge
      </div>
      <div v-for="kn in knowledge" :key="kn.name"
           class="mnemonic-item">
        <div class="mnemonic-header">
          <span class="mnemonic-file">{{ kn.name }}</span>
          <Timestamp :value="kn.timestamp"/>
        </div>
        <TransitionGroup name="highlight" tag="div">
          <Entry
              v-for="(entry, i) in (kn.content as MnemonicEntry[])"
              :key="entry.id ?? i"
              :cue="extractCue(entry.id ?? '')"
