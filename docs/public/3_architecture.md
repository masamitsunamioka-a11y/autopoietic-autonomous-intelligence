...
Message hierarchy (specification): `Message` is the common root marker interface.
`Command` (runtime) and `Event` (anticorruption) both extend `Message`.

Command types (runtime): `CompensateNeural`, `ConserveNeural`,
`EncodeEpisode`, `DecayEpisode`, `EncodeKnowledge`, `DecayKnowledge`.

Event types (anticorruption): `AreaCreated`, `AreaUpdated`, `AreaRemoved`, `NeuronCreated`,
`NeuronRemoved`, `EffectorCreated`, `TraceEncoded`, `StrengthDecayed`, `TraceRemoved`.

EventStore is the single point of persistence and publication (Greg Young pattern).
CommandHandlers delegate to EventStore.save(), which persists events then publishes
them internally via EventPublisher. This eliminates the two-phase commit risk.
...
├── adapters/driven/service   [runtime] (Effector impls)
│   └── specification *
├── runtime                 [runtime]
│   └── specification *
...
| **service**        | Concrete `*Effector.java`, may be runtime-generated by Autopoiesis            |
