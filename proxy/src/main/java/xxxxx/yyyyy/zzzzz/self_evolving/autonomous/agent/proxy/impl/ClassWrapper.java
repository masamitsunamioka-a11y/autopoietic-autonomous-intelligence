...
import xxxxx.yyyyy.zzzzz.self_evolving.autonomous.agent.proxy.AnnotatedType;
...
public class ClassWrapper<T> implements AnnotatedType<T> {
    ...
    private Set<Annotation> filterMetaAnnotation(Annotation[] annotations, Class<? extends Annotation> metaAnnotationType) {
        return Arrays.stream(annotations)
                .filter(xyz -> xyz.annotationType().isAnnotationPresent(metaAnnotationType))
                .collect(Collectors.toSet());
    }

    private Set<Class<? extends Annotation>> filterMetaAnnotationType(Annotation[] annotations, Class<? extends Annotation> metaAnnotationType) {
        return Arrays.stream(annotations)
