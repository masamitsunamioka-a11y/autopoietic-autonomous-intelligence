...
public class RegexPromptBuilder implements PromptBuilder {
    private static final String GUARDRAILS_ST = "guardrails.st";
    private static final String REASONING_ST = "reasoning.st";
    private static final String ROUTING_ST = "routing.st";
    private static final String EVOLVING_ST = "evolution.st";
    ...
    @Override
    public Session reasoning() {
        String template = this.promptRepository.findByName(REASONING_ST);
        return new InnerSession(template);
    }

    @Override
    public Session routing() {
        String template = this.promptRepository.findByName(ROUTING_ST);
        return new InnerSession(template);
    }

    @Override
    public Session evolution() {
        String template = this.promptRepository.findByName(EVOLVING_ST);
        return new InnerSession(template);
    }
    ...
    private class InnerSession implements Session {
        ...
        public Session guardrails() {
            return this.bind("guardrails", promptRepository.findByName(GUARDRAILS_ST));
        }
